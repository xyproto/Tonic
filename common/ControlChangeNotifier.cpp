...
namespace Tonic_ {
    ...
    void ControlChangeNotifier_::computeOutput(const SynthesisContext_& context)
    {
        ...
        if (output_.triggered) {
            outputReadyToBeSentToUI
                = false; // maybe silly, but a tiny bit of atomicness/thread safety here
    ...
    void ControlChangeNotifier_::sendControlChangesToSubscribers()
    {
        if (outputReadyToBeSentToUI) {
            for (vector<ControlChangeSubscriber*>::iterator it = subscribers.begin();
    ...
    {
        subscribers.erase(
            std::remove(subscribers.begin(), subscribers.end(), sub), subscribers.end());
    }
