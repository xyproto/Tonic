...
namespace Tonic_ {
    ...
    void StereoDelay_::initialize(
        float leftDelayArg, float rightDelayArg, float maxDelayLeft, float maxDelayRight)
    {

        if (maxDelayLeft <= 0)
            maxDelayLeft = leftDelayArg * 1.5;
        if (maxDelayRight <= 0)
            maxDelayRight = rightDelayArg * 1.5;
...
StereoDelay::StereoDelay(
    float leftDelay, float rightDelay, float maxDelayLeft, float maxDelayRight)
{
