...
namespace Tonic_ {
    ...
    RingBufferWriter_::~RingBufferWriter_() { s_RingBuffers_.removeObjectNamed(tableName_); }

    void RingBufferWriter_::initRingBuffer(
        string name, unsigned int nFrames, unsigned int nChannels)
    {
        ...
        if (s_RingBuffers_.containsObjectNamed(name)) {
            warning("There is already a ring buffer named " + name
                + " open for writing. It will be replaced with this one.");
        }
#endif
    ...
    void RingBufferWriter_::reset() { ringBuffer_.reset(); }
}
