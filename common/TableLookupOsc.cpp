...
namespace Tonic_ {
    ...
    void TableLookupOsc_::reset() { phase_ = 0.0f; }
    ...
    void TableLookupOsc_::setLookupTable(SampleTable table)
    {
        ...
        if (!isPowerOf2((unsigned int)table.size() - 1, &nearestPo2)) {
            ...
            warning("TableLookUpOsc lookup tables must have a (power-of-two + 1) number of "
                    "samples (example 2049 or 4097). Resizing to nearest power-of-two + 1");
