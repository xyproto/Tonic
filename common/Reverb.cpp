...
namespace Tonic_ {
    ...
    static const TonicFloat combTimeScales_[TONIC_REVERB_N_COMBS]
        = { 1.17, 1.12, 1.02, 0.97, 0.95, 0.88, 0.84, 0.82 };
    static const TonicFloat allpassTimes_[TONIC_REVERB_N_ALLPASS]
        = { 0.0051, 0.010, 0.012, 0.00833 };
    ...
    Reverb_::Reverb_()
    {
        ...
        for (unsigned int i = 0; i < TONIC_REVERB_N_COMBS; i++) {
            ...
            combFilters_[TONIC_LEFT].push_back(
                FilteredFBCombFilter6(0.01f, 0.125f).delayTime(delayL).scaleFactor(scaleL));
            combFilters_[TONIC_RIGHT].push_back(
                FilteredFBCombFilter6(0.01f, 0.125f).delayTime(delayR).scaleFactor(scaleR));
        }

        for (unsigned int i = 0; i < TONIC_REVERB_N_ALLPASS; i++) {
            allpassFilters_[TONIC_LEFT].push_back(
                ImpulseDiffuserAllpass(allpassTimes_[i], TONIC_REVERB_ALLPASS_COEF));
            allpassFilters_[TONIC_RIGHT].push_back(ImpulseDiffuserAllpass(
                allpassTimes_[i] + TONIC_REVERB_STEREO_SPREAD, TONIC_REVERB_ALLPASS_COEF));
        }
    ...
    void Reverb_::updateDelayTimes(const SynthesisContext_& context)
    {
        ...
        if (densityOutput.triggered || shapeOutput.triggered || sizeOutput.triggered) {
            ...
            TonicFloat wDist1 = map(
                size, 0.f, 1.f, TONIC_REVERB_MIN_WALL_DIST, TONIC_REVERB_MAX_WALL_DIST, true);
            TonicFloat wDist2 = map(size * (1.1f - shape), 0.f, 1.f, TONIC_REVERB_MIN_WALL_DIST,
                TONIC_REVERB_MAX_WALL_DIST, true);

            unsigned int nTaps = (unsigned int)map(
                densityOutput.value, 0.f, 1.f, TONIC_REVERB_MIN_TAPS, TONIC_REVERB_MAX_TAPS, true);
            TonicFloat tapScale = 1.0f / max(2.f, sqrtf(nTaps));
            for (unsigned int i = 0; i < nTaps; i++) {

                TonicFloat dist = (i % 2 == 0 ? wDist1 : wDist2)
                    * (1.0f + randomFloat(-TONIC_REVERB_FUDGE_AMT, TONIC_REVERB_FUDGE_AMT));
        ...
        if (sizeOutput.triggered || decayOutput.triggered) {
            ...
            TonicFloat baseCombDelayTime = map(sizeOutput.value, 0.f, 1.f,
                TONIC_REVERB_MIN_COMB_TIME, TONIC_REVERB_MAX_COMB_TIME, true);

            for (unsigned int i = 0; i < TONIC_REVERB_N_COMBS; i++) {

                TonicFloat scaledDelayTime
                    = combTimeScales_[i % TONIC_REVERB_N_COMBS] * baseCombDelayTime;
                combFilterDelayTimes_[TONIC_LEFT][i].value(scaledDelayTime);
                combFilterDelayTimes_[TONIC_RIGHT][i].value(
                    scaledDelayTime + TONIC_REVERB_STEREO_SPREAD);
                combFilterScaleFactors_[TONIC_LEFT][i].value(
                    powf(10.f, (-3.0f * scaledDelayTime / decayTime)));
                combFilterScaleFactors_[TONIC_RIGHT][i].value(powf(
                    10.f, (-3.0f * (scaledDelayTime + TONIC_REVERB_STEREO_SPREAD) / decayTime)));
            }
        }
    }
