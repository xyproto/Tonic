...
namespace Tonic_ {
    ...
    inline void ControlSwitcher_::computeOutput(const SynthesisContext_& context)
    {
        ...
        if (inputs_.size() > 0) {
            ...
            // Check to see if the value changed. If it hasn't changed, ignore it, lest we
            // overwrite a value set by one of the triggers.
            ...
            for (std::map<int, ControlGenerator>::iterator it = triggers_.begin();
                 it != triggers_.end(); it++) {
            ...
            int cleanedInput = doesWrapOut.value
                ? currentInputIndex_ % inputs_.size()
                : clamp(currentInputIndex_, 0, inputs_.size() - 1);

            int index = 0;
            for (vector<ControlGenerator>::iterator it = inputs_.begin(); it != inputs_.end();
                 it++) {
            ...
            if (doesWrapOut.value) {
                // Do the add after wrap thing. Mostly useful for going to scale degrees in higher
                // octaves
    ...
    void ControlSwitcher_::addInput(ControlGenerator input) { inputs_.push_back(input); }

    void ControlSwitcher_::setInputs(vector<ControlGenerator> inputs) { inputs_ = inputs; }

    void ControlSwitcher_::setDoesWrap(ControlGenerator doesWrap) { doesWrap_ = doesWrap; }
