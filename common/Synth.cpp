...
namespace Tonic_ {
    ...
    void Synth_::setParameter(string name, float value, bool normalized)
    {
        ...
        if (parameters_.find(name) != parameters_.end()) {
            ...
            error("message: " + name
                + " was not registered. You can register a message using Synth::addParameter.");
        }
    }
    ...
    ControlParameter Synth_::addParameter(string name, TonicFloat initialValue)
    {
        if (parameters_.find(name) == parameters_.end()) {
            ControlParameter param
                = ControlParameter().name(name).value(initialValue).displayName(name);
    ...
    vector<ControlParameter> Synth_::getParameters()
    {
        ...
        for (std::vector<string>::iterator it = orderedParameterNames_.begin();
             it != orderedParameterNames_.end(); it++) {
    ...
    {
        for (vector<ControlChangeNotifier>::iterator it = controlChangeNotifiersList_.begin();
             it != controlChangeNotifiersList_.end(); it++) {
