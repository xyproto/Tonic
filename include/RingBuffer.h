...
namespace Tonic_ {
    ...
    inline void RingBuffer_::write(TonicFloat* data, unsigned int nFrames, unsigned int nChannels)
    {
        ...
        if (finalWriteHead >= readHead_
            && (writeHead_ < readHead_ || finalWriteHead < writeHead_)) {
...
//! Like a SampleTable_, but with counters for over/underrun detection and easy synchronous
//! read/write
//  TODO: Maybe should template the SampleTable smart pointer instead of statically casting the
//  object?
...
public:
    ...
    void read(TonicFrames& outFrames) { static_cast<Tonic_::RingBuffer_*>(obj)->read(outFrames); }

    void reset() { static_cast<Tonic_::RingBuffer_*>(obj)->reset(); }
};
...
public:
    RingBufferWriter()
        : TonicSmartPointer<Tonic_::RingBufferWriter_>(new Tonic_::RingBufferWriter_)
    {
    }
