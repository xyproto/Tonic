...
public:
    ...
    Generator operator[](unsigned int index) { return gen()->getInput(index); }

    unsigned long numInputs() { return gen()->numInputs(); }
};
...
static Adder operator+(Generator a, ControlGenerator b) { return a + FixedValue().setValue(b); }

static Adder operator+(ControlGenerator a, Generator b) { return FixedValue().setValue(a) + b; }

// Add an Adder and a ControlGenerator

static Adder operator+(Adder a, ControlGenerator b) { return a + FixedValue().setValue(b); }

static Adder operator+(ControlGenerator a, Adder b) { return FixedValue().setValue(a) + b; }
...
namespace Tonic_ {
    ...
    inline void Multiplier_::computeSynthesisBlock(const SynthesisContext_& context)
    {
        ...
        // for additional generators, use the workspace stkframes for tick, and multiply it into
        // the frames argument
...
static Multiplier operator*(float a, Generator b) { return FixedValue(a) * b; }

static Multiplier operator*(Generator a, float b) { return a * FixedValue(b); }
...
static Divider operator/(Generator a, ControlGenerator b) { return a / FixedValue().setValue(b); }

static Divider operator/(ControlGenerator a, Generator b) { return FixedValue().setValue(a) / b; }
}
