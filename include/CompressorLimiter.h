...
namespace Tonic_ {
    ...
    inline void Compressor_::tickThrough(
        TonicFrames& inFrames, TonicFrames& outFrames, const SynthesisContext_& context)
    {
...
// clip to threshold in worst case (minor distortion introduced but much preferable to wrapping
// distortion)
#ifdef USE_APPLE_ACCELERATE
            float negThresh = -threshold;
            vDSP_vclip(&outputFrames_[0], 1, &negThresh, &threshold, &outputFrames_[0], 1,
                outputFrames_.size());
#else
            outptr = &outputFrames_[0];
...
public:
    Compressor(float threshold = 0.5, float ratio = 2, float attack = 0.001, float release = 0.05,
        float lookahead = 0.001);
    ...
    void setIsStereo(bool isStereo) { this->gen()->setIsStereo(isStereo); }

    TONIC_MAKE_CTRL_GEN_SETTERS(Compressor, attack, setAttack);
    TONIC_MAKE_CTRL_GEN_SETTERS(Compressor, release, setRelease);
    TONIC_MAKE_CTRL_GEN_SETTERS(
        Compressor, threshold, setThreshold); // LINEAR - use dBToLin to convert from dB
