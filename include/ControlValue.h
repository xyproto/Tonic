...
namespace Tonic_ {
    ...
    inline void ControlValue_::computeOutput(const SynthesisContext_& context)
    {
        ...
        changed_ = context.forceNewOutput; // if new output forced, don't reset changed status
                                           // until next tick
...
public:
    ControlValue(TonicFloat value = 0) { gen()->setValue(value); }
    ...
    float getValue() { return gen()->getValue(); }
};
}
