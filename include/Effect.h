...
namespace Tonic_ {
    ...
    public:
        ...
        */
        virtual void tickThrough(
            TonicFrames& inFrames, TonicFrames& outFrames, const SynthesisContext_& context);
    };
    ...
    inline void Effect_::tickThrough(
        TonicFrames& inFrames, TonicFrames& outFrames, const SynthesisContext_& context)
    {
...
public:
    // This cast is not safe - up to implementation to ensure that templated EffectType_ is
    // actually an Effect_ subclass
    ...
    void tickThrough(TonicFrames& inFrames, const Tonic_::SynthesisContext_& context)
    { // ticks in-place
        this->gen()->tickThrough(inFrames, inFrames, context);
    }

    void tickThrough(
        TonicFrames& inFrames, TonicFrames& outFrames, const Tonic_::SynthesisContext_& context)
    {
        this->gen()->tickThrough(inFrames, outFrames, context);
    }

    void setIsStereoInput(bool isStereoInput) { this->gen()->setIsStereoInput(isStereoInput); }
...
namespace Tonic_ {
    ...
    // WetDryEffect_ extends the basic functionality of Effect_ but also adds a wet/dry mix to the
    // basic tick() cycle
    ...
    inline void WetDryEffect_::tickThrough(
        TonicFrames& inFrames, TonicFrames& outFrames, const SynthesisContext_& context)
    {
