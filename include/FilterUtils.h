...
*/
inline static void bltCoef(TonicFloat b2, TonicFloat b1, TonicFloat b0, TonicFloat a1,
    TonicFloat a0, TonicFloat fc, TonicFloat* coef_out)
{
...
//! Biquad_ is an IIR biquad filter which provides a base object on which to build more advanced
//! filters
class Biquad {
...
public:
    ...
    void setCoefficients(
        TonicFloat b0, TonicFloat b1, TonicFloat b2, TonicFloat a1, TonicFloat a2);
    void setCoefficients(TonicFloat* newCoef);

    void filter(TonicFrames& inFrames, TonicFrames& outFrames);
};

inline void Biquad::setCoefficients(
    TonicFloat b0, TonicFloat b1, TonicFloat b2, TonicFloat a1, TonicFloat a2)
{
    coef_[0] = b0;
    coef_[1] = b1;
...
inline void Biquad::filter(TonicFrames& inFrames, TonicFrames& outFrames)
{
    ...
    // initialize vectors
    memcpy(&inputVec_[0], &inputVec_(kSynthesisBlockSize, 0),
        2 * inputVec_.channels() * sizeof(TonicFloat));
    memcpy(&inputVec_(2, 0), &inFrames[0], inFrames.size() * sizeof(TonicFloat));
    memcpy(&outputVec_[0], &outputVec_(kSynthesisBlockSize, 0),
        2 * outputVec_.channels() * sizeof(TonicFloat));
    ...
    for (unsigned int c = 0; c < stride; c++) {
        vDSP_deq22(
            &inputVec_[0] + c, stride, coef_, &outputVec_[0] + c, stride, kSynthesisBlockSize);
    }
#else
    ...
    for (unsigned int c = 0; c < stride; c++) {
        ...
        for (unsigned int i = 0; i < kSynthesisBlockSize; i++) {
            *out = *(in)*coef_[0] + *(in - stride) * coef_[1] + *(in - 2 * stride) * coef_[2]
                - *(out - stride) * coef_[3] - *(out - 2 * stride) * coef_[4];
            in += stride;
            out += stride;
        }
    }
