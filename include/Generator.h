...
public:
    Generator(Tonic_::Generator_* gen = new Tonic_::Generator_)
        : TonicSmartPointer<Tonic_::Generator_>(gen)
    {
    }

    inline bool isStereoOutput() { return obj->isStereoOutput(); }
...
// Each generator should have three flavors of setter -- one that accepts a float, one that accepts
// a ControlGenerator, and one that accepts a Generator. This macro will automatically build those
// three setters
