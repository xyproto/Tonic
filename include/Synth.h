...
namespace Tonic_ {
    ...
    protected:
        ...
        // ControlGenerators that may not be part of the synthesis graph, but should be ticked
        // anyway
    ...
    public:
        ...
        void forceNewOutput() { synthContext_.forceNewOutput = true; }
    ...
    inline void Synth_::computeSynthesisBlock(const SynthesisContext_& context)
    {
        ...
        for (vector<ControlGenerator>::iterator it = auxControlGenerators_.begin();
             it != auxControlGenerators_.end(); it++) {
            it->tick(context);
        }
...
public:
    ...
    const Generator getOutputGen() { return gen()->getOutputGen(); }

    //! Set whether synth uses dynamic limiter to prevent clipping/wrapping. Defaults to true.
    void setLimitOutput(bool shouldLimit) { gen()->setLimitOutput(shouldLimit); }
    ...
    //! Add a ControlParameter initialized elsewhere. Will overwrite existing parameter with the
    //! same name.
    void addParameter(ControlParameter parameter) { gen()->addParameter(parameter); }

    //! Add all the parameters from another synth
    void addParametersFromSynth(Synth synth) { gen()->addParametersFromSynth(synth); }

    /*!
      Returns a ControlConditioner which accepts an input and a ControlChangeSubscriber (supplied
      by the UI). When the input value changes, ControlChangeSubscriber::messageRecieved is called.
      You would typically call this method inside a synth definition if you have a ControlGenerator
      whose value you want to make accessible to the UI thread. You then subscribe to these events
      by creating a subclass of ControlChangeSubscriber and passing a pointer to that object to
      Synth::addControlChangeSubscriber.
    */
    ...
    //! Add a ControlGenerator to a list of objects which will be ticked regardless of whether
    //! they're part of the synthesis graph or not.
    void addAuxControlGenerator(ControlGenerator generator)
    {
        gen()->lockMutex();
    ...
    //! Add an object which will be notified when a particular ControlChangeNotifier changes value
    //! or is triggered.
    void addControlChangeSubscriber(string name, ControlChangeSubscriber* resp)
    {
    ...
    //! Add an object which will be notified when any ControlChangeNotifier changes value or is
    //! triggered.
    void addControlChangeSubscriber(ControlChangeSubscriber* resp)
    {
    ...
    */
    void sendControlChangesToSubscribers() { gen()->sendControlChangesToSubscribers(); }
    ...
    vector<ControlParameter> getParameters() { return gen()->getParameters(); }
...
template <typename T> Synth createSynth() { return T(); }
...
struct SynthFactory {
    ...
    static Synth createInstance(std::string const& s)
    {
        ...
        if (it == getMap()->end()) {
            ...
            error("Error creating synth. Synth named \"" + s
                + "\" not found. Existing registered synths are: \n" + synthsList);
...
template <typename T> struct SourceRegister : SynthFactory {
    SourceRegister(std::string const& s) { getMap()->insert(std::make_pair(s, &createSynth<T>)); }
};
}
