...
public:
    //! Allocation parameters are binding. No post-allocation resizing or modifying channel layout
    //! (for now anyway).
    /*!
    ...
    */
    ...
    /*!
        NOTE: No bounds checking on channel index. Improper use will result in access outside array
       bounds.
    */
    ...
    inline TonicFloat tickOut(float delayTime, unsigned int channel = 0)
    {
        ...
        if (interpolates_) {
            ...
            if (idx_a >= size_)
                idx_a -= size_; // this happens occasionally due to floating point rounding
