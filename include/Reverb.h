...
namespace Tonic_ {
    ...
    */
    ...
    /*!
        ...
        TODO:
        - Reverb fb comb cutoff setting parameters should be normalized value, not Hz. Not a "true"
       cutoff.
    ...
    inline void Reverb_::computeSynthesisBlock(const SynthesisContext_& context)
    {
        ...
        for (unsigned int i = 0; i < combFilters_[TONIC_LEFT].size(); i++) {
            combFilters_[TONIC_LEFT][i].tickThrough(
                workspaceFrames_[0], workspaceFrames_[1], context);
            preOutputFrames_[TONIC_LEFT] += workspaceFrames_[1];
            combFilters_[TONIC_RIGHT][i].tickThrough(
                workspaceFrames_[0], workspaceFrames_[1], context);
        ...
        TonicFloat normValue
            = (1.0f / (1.0f + spreadValue)) * 0.04f; // scale back levels quite a bit
