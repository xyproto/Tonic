...
namespace Tonic_ {
    ...
    {
        memcpy(&outputFrames_[0], &buffer_.dataPointer()[startSample],
            numSamples * sizeof(TonicFloat));
    }
}

/*!
  Simply plays back a buffer. "loop" parameter works, but doesn't wrap between ticks, so mostly
  likely you'll wind up with a few zeroes at the end of the last buffer if you're looping. In other
  words, buffer lenghts are rounded up to the nearest kSynthesisBlockSize
