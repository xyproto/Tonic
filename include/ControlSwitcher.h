...
/*!
    ControlSwitcher allows you to switch between an unlimited number of inputs (added via
   addInput). The "active" input is controlled by the value of inputIndex and any iputs passed via
   triggerForIndex. If the input index changes, the object will send a trigger (formerly known as
   "hasChanged") message. Currently, the inputIndex must send a trigger message every time it
   changes, or the change won't be registered.
*/
...
public:
    ...
    //! Useful if you want to use this to map to scale degrees. For midi notes, you can use 12 to
    //! jump to the next octave when it wraps.
