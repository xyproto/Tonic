...
namespace Tonic_ {
    ...
    /*!
     BufferFillers provide a high-level interface for combinations of generators, and can be used
     to fill arbitraryly large buffers.
    ...
    inline void BufferFiller_::lockMutex() { TONIC_MUTEX_LOCK(mutex_); }

    inline void BufferFiller_::unlockMutex() { TONIC_MUTEX_UNLOCK(mutex_); }
    ...
    inline void BufferFiller_::fillBufferOfFloats(
        float* outData, unsigned int numFrames, unsigned int numChannels)
    {
        ...
        if (numChannels > outputFrames_.channels())
            error("Mismatch in channels sent to Synth::fillBufferOfFloats", true);
#endif
...
public:
     ...
     */
    inline void fillBufferOfFloats(
        float* outData, unsigned int numFrames, unsigned int numChannels)
    {
        static_cast<Tonic_::BufferFiller_*>(obj)->fillBufferOfFloats(
            outData, numFrames, numChannels);
    }
};

template <class GenType> class TemplatedBufferFiller : public BufferFiller {
protected:
    GenType* gen() { return static_cast<GenType*>(obj); }
