...
public:
    ...
    */
    ...
    //! Resize self to represent the specified number of channels and frames and perform element
    //! initialization.
    /*!
      Changes the size of self based on the number of frames and
...
inline TonicFloat& TonicFrames ::operator()(size_t frame, unsigned int channel)
{
    ...
    if (frame >= nFrames_ || channel >= nChannels_) {
        std::ostringstream error;
        error << "TonicFrames::operator(): invalid frame (" << frame << ") or channel (" << channel
              << ") value!";
        Tonic::error(error.str(), true);
    }
#endif
...
inline void TonicFrames::clear() { memset(data_, 0, size_ * sizeof(TonicFloat)); }
