...
namespace Tonic_ {
    ...
    inline void ControlDelay_::computeOutput(const SynthesisContext_& context)
    {
        ...
        if (delayTimeOutput.triggered) {
            ...
            unsigned delayBlocks
                = max(delayTimeOutput.value * sampleRate() / kSynthesisBlockSize, 1);

#ifdef TONIC_DEBUG
            if (delayBlocks >= maxDelay_) {
                debug("ControlDelay: delay time greater than maximum delay (defaults to 1 scond). "
                      "Use constructor to set max delay -- ex. ControlDelay(2.0))");
            }
#endif
