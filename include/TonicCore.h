...
// Uncomment or define in your build configuration to log debug messages and perform extra debug
// checks #define TONIC_DEBUG

// Determine if C++11 is available. If not, some synths cannot be used. (applies to oF demos,
// mostly)
...
inline static float log2(float n) { return log(n) / log(2); }
#endif
...
// TODO: Any other non-SSE platforms that allow denormals by default? ARM-based targets (iPhone,
// for example) do not.
...
/*! !!!: CHANGING WHILE RUNNING WILL RESULT IN UNDEFINED BEHAVIOR. MUST BE SET PRIOR TO OBJECT
 * ALLOCATION. */
static void setSampleRate(TonicFloat sampleRate) { Tonic_::sampleRate_ = sampleRate; }

//! Return sample rate
static TonicFloat sampleRate() { return Tonic_::sampleRate_; };
...
//-- Vector Math --
inline static void vcopy(TonicFloat* dst, unsigned int dst_str, const TonicFloat* src,
    unsigned int src_str, unsigned int length)
{
...
inline static TonicFloat max(TonicFloat a, TonicFloat b) { return (a > b ? a : b); }

inline static TonicFloat min(TonicFloat a, TonicFloat b) { return (a < b ? a : b); }
...
inline static TonicFloat map(TonicFloat val, TonicFloat inMin, TonicFloat inMax, TonicFloat outMin,
    TonicFloat outMax, bool clamp = true)
{
...
//! Takes linear value 0-1, maps to logarithmic value (base logBase) scaled to min-max. Useful for
//! making faders.
...
{
    return map(log10f(map(logValue, min, max, 0.0001f, 1.f, true)), TONIC_LOG_MAP_BASEVAL, 0.f,
        0.f, 1.f, true);
}
...
inline static TonicFloat mtof(TonicFloat nn) { return 440.0f * powf(2.0f, (nn - 69.0f) / 12.0f); }
...
/*
  Using 0 dBFS as 1.0
*/
inline static TonicFloat linTodB(TonicFloat lv) { return 20.0f * log10f(max(0, lv)); }

inline static TonicFloat dBToLin(TonicFloat dBFS) { return powf(10.f, (dBFS / 20.0f)); }

// -- Misc --

inline static TonicFloat randomSample() { return ((TonicFloat)rand() / RAND_MAX) * 2.0f - 1.0f; }
...
public:
    TonicException(string const& error)
        : runtime_error(error){};
};
...
static void warning(string message) { printf("Tonic::warning: %s\n", message.c_str()); }
...
//! Dictionary helper class for registering objects by name. For correct usage, objects should be
//! Smart Pointers.
template <class T> class TonicDictionary {
...
public:
    ...
    void insertObject(string name, T object) { dictionaryMap_[name] = object; }
    ...
    //! Returns object with given name. Returns new object if no object has been set for name, does
    //! not insert it.
...
template <class T> class TonicSmartPointer {
...
public:
    ...
    TonicSmartPointer(T* initObj)
        : obj(initObj)
        , pcount(initObj ? new int(1) : NULL)
    {
    }

    TonicSmartPointer(const TonicSmartPointer& r)
        : obj(r.obj)
        , pcount(r.pcount)
    {
        retain();
    ...
    bool operator==(const TonicSmartPointer& r) { return obj == r.obj; }
};
