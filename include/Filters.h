...
namespace Tonic_ {
    ...
    protected:
        ...
        virtual void applyFilter(TonicFloat cutoff, TonicFloat Q, const SynthesisContext_& context)
            = 0;
    ...
    inline void Filter_::computeSynthesisBlock(const SynthesisContext_& context)
    {
        ...
        // For now only using first frame of output. Setting coefficients each frame is very
        // inefficient. Updating cutoff every 64-samples is typically fast enough to avoid audible
        // artifacts when sweeping filters.
